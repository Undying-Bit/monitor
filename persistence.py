...
_CREATE_TRIGGERS = [
    """
    CREATE TRIGGER IF NOT EXISTS trg_pruebas_insert
    AFTER INSERT ON normalized_events
    WHEN NEW.report_slot IS NULL AND NEW.tone = 0 AND NEW.event_type != 'EQW'
    BEGIN
        INSERT INTO pruebas
        ...
        VALUES
            (NEW.id, NEW.source, NEW.station_id, NEW.station_name, NEW.station_code,
             NEW.report_date, NEW.report_slot, NEW.event_type, NEW.event_class, NEW.tone,
             NEW.event_time_local, NEW.event_time_utc, NEW.confidence_score, NEW.parser_version,
             NEW.transmitter_code, NEW.phone_number, NEW.channel, NEW.payload_json, datetime('now'));
    END;
    """,
    """
    CREATE TRIGGER IF NOT EXISTS trg_alertas_insert
    AFTER INSERT ON normalized_events
    WHEN NEW.report_slot IS NULL AND NEW.event_type = 'EQW'
    BEGIN
        INSERT INTO alertas
            (normalized_event_id, source, station_id, station_name, station_code,
             report_date, report_slot, event_type, event_class, tone,
    ...
    """,
    """
    CREATE TRIGGER IF NOT EXISTS trg_pruebas_delete
    AFTER DELETE ON normalized_events
    BEGIN
        DELETE FROM pruebas WHERE normalized_event_id = OLD.id;
        DELETE FROM alertas WHERE normalized_event_id = OLD.id;
    END;
    """,
]
...
async def init_db(db_path: Path | None = None) -> None:
    ...
    async with open_db(db_path) as db:
        ...
        for trig_sql in _CREATE_TRIGGERS:
            await db.execute(trig_sql)
        await db.commit()
