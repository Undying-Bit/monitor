...
class SerialIngress:
    ...
    def __init__(self, queue: asyncio.Queue[RawEvent], ports: list[str]) -> None:
        ...
        self._queue_log_lock = threading.Lock()
        self._last_queue_pressure_log = 0.0
                    ...
                    while not self._stop_event.is_set():
                        ...
                        while True:
                            ...
                            if term_index != -1:
                                ...
                                self._enqueue_blocking(loop, raw, port)
                                continue
    ...
    def _enqueue_blocking(
        ...
        self._maybe_log_queue_pressure(port)
        ...
        while True:
            try:
                ...
                logger.info(
                    "Skipping serial enqueue for %s because the event loop is closed",
                    port,
                )
                return

    def _maybe_log_queue_pressure(self, port: str) -> None:
        ...
        if qsize < max(1, int(maxsize * 0.8)):
            return
        with self._queue_log_lock:
            now = time.monotonic()
            if now - self._last_queue_pressure_log < 5.0:
                return
            self._last_queue_pressure_log = now
        logger.warning(
            "Serial queue pressure on %s: %d/%d buffered",
            port,
            qsize,
            maxsize,
        )
